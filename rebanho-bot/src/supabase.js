...
async function salvarAnimais(fazenda, animais, loteId) {
  ...
  for (const animal of animais) {
    ...
    const { data: animalSalvo, error: errAnimal } = await supabase
      .from('animais')
      .upsert({
        fazenda, brinco: animal.brinco || null, chip: animal.chip || null,
        sexo: animal.sexo || null, raca: animal.raca || null,
        categoria: animal.categoria || null, lote_id: loteId || null,
        status: animal.evento === 'venda' ? 'vendido' : animal.evento === 'morte' ? 'morto' : 'ativo',
    ...
    if (animal.evento && loteId) {
      const tipoMap = { nascimento:'entrada_nascimento', compra:'entrada_compra',
        transferencia:'entrada_transferencia', venda:'saida_venda', morte:'saida_morte', pesagem:'pesagem' }
      await supabase.from('movimentacoes_lote').insert({
        fazenda, lote_id: loteId, animal_id: animalSalvo?.id || null,
        tipo: tipoMap[animal.evento] || 'entrada_compra',
        peso: animal.peso || null, observacoes: animal.observacoes || null,
      })
    }
    salvos++
  }
...
async function salvarRebanho(dados, transcricao, whatsappDe) {
  ...
  if (!mes || !ano) throw new Error('Mes e ano nao identificados. Por favor, mencione o periodo.')

  const loteId = await buscarOuCriarLote(fazenda, dados.lote_nome, {
    pasto: dados.lote_pasto, finalidade: dados.lote_finalidade, raca: dados.lote_raca,
  })

  // Buscar ou criar registro mensal — select-first evita problema com indice parcial
  const diaVal = dia || null
  const queryBusca = supabase
    .from('rebanho_mensal').select('id')
    .eq('fazenda', fazenda).eq('ano', ano).eq('mes', mes)
  if (diaVal !== null) { queryBusca.eq('dia', diaVal) }
  else { queryBusca.is('dia', null) }

  const { data: existente } = await queryBusca.single()

  let mensal, errMensal
  if (existente) {
    await supabase.from('rebanho_mensal')
      .update({ transcricao, whatsapp_de: whatsappDe, lote_id: loteId })
      .eq('id', existente.id)
    mensal = existente
  } else {
    const { data: novo, error: err } = await supabase
      .from('rebanho_mensal')
      .insert({ dia: diaVal, mes, ano, fazenda, transcricao, whatsapp_de: whatsappDe, lote_id: loteId })
      .select('id').single()
    errMensal = err
    mensal = novo
  }
  ...
  for (const cat of categorias) {
    ...
    if (existente) {
      ...
      await supabase.from('rebanho_categoria').insert({
        rebanho_id: rebanhoId, item: cat.item, discriminacao: cat.discriminacao, sexo: cat.sexo,
        existencia_anterior: cat.existencia_anterior||0, existencia_atual: cat.existencia_atual||0,
        entrada_compra: cat.entrada_compra||0, entrada_mudanca_cat: cat.entrada_mudanca_cat||0,
        entrada_desmama: cat.entrada_desmama||0, entrada_nascimento: cat.entrada_nascimento||0,
        entrada_transferencia: cat.entrada_transferencia||0, entrada_total: cat.entrada_total||0,
        saida_abate: cat.saida_abate||0, saida_venda: cat.saida_venda||0, saida_morte: cat.saida_morte||0,
        saida_desmama: cat.saida_desmama||0, saida_mudanca_cat: cat.saida_mudanca_cat||0,
        saida_transferencia: cat.saida_transferencia||0, saida_total: cat.saida_total||0,
  ...
  console.log('Salvo: ' + (diaVal||'--') + '/' + mes + '/' + ano + ' | lote: ' + (dados.lote_nome||'Geral') + ' | cats: ' + categorias.length + ' | animais: ' + animaisSalvos)
...
async function buscarResumoMensal(limite = 12) {
  const { data, error } = await supabase.from('vw_resumo_mensal').select('*').limit(limite)
  if (error) throw new Error('Erro ao buscar resumo: ' + error.message)
  return data
}

async function buscarResumoPorLote(fazenda = 'Grupo Ricci') {
  const { data, error } = await supabase.from('vw_resumo_lote').select('*').eq('fazenda', fazenda)
